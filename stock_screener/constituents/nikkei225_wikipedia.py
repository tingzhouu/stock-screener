...
def _to_yahoo_tokyo_ticker(raw_symbol: str) -> str | None:
    token = str(raw_symbol).strip().upper()
    if not token or token == "NAN":
        return None

    compact = token.replace(" ", "")
    match_with_suffix = re.search(r"(\d{4})\.T\b", compact)
    if match_with_suffix:
        return f"{match_with_suffix.group(1)}.T"

    match_code = re.search(r"\b(\d{4})\b", token)
    if match_code:
        return f"{match_code.group(1)}.T"
...
def _parse_list_constituents(page_html: str) -> List[Constituent]:
    ...
    for li in tree.xpath("//li"):
        ...
        if "TYO" not in text.upper():
            continue
        ticker = _to_yahoo_tokyo_ticker(text)
        ...
        anchors = li.xpath("./a")
        if anchors:
            first = anchors[0].text_content().strip()
            if first and first.upper() != "TYO":
                company = first
...
def get_nikkei225_constituents(min_count: int = 180) -> List[Constituent]:
    ...
    for df in tables:
        ...
        for _, row in df.iterrows():
            ticker = _to_yahoo_tokyo_ticker(str(row[symbol_col]))
            if not ticker or ticker in seen:
                continue
            ...
            company = re.sub(r"\[[0-9]+\]", "", company).strip()
            seen.add(ticker)
            out.append(Constituent(ticker=ticker, company=company))
    ...
    raise ValueError("Could not parse NIKKEI225 constituents from table/list source")
